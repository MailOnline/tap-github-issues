    ...
    return Promise.all(repoTests.map(function (test) {
      ...
      if (existing) {
        if (existing.state == 'open') {
          if (ok) {
            console.log('ok (closing...):', repo, rule);
            ...
            if (passedDays >= opts.remind) {
              console.log('not ok (reminding...):', repo, rule);
              p = github.issue.comment(repo, existing, issue.comments.update, opts);
            } else {
              console.log('not ok (recent):', repo, rule);
            }
          }
          ...
          if (ok) {
            ...
            console.log('not ok (re-opening...):', repo, rule);
        ...
        if (ok) {
          ...
          console.log('not ok (creating...):', repo, rule);
