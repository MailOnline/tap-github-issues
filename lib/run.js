...
module.exports = (_argv, stream=process.stdin, doExit) => {
  return new Promise((resolve, reject) => {
    ...
    const p = new Parser({passes: true}, function (results) {
      processResults(results, opts).then(resolve, reject);
    });
...
async function processResults(results, opts) {
  const tests = prepareTests(results);
  let newIssues = 0, closedIsues = 0, reopenedIssues = 0, remindedIssues = 0;
  for (const repo in tests)
    await processRepo(repo, tests[repo]);

  const passes = results.passes.length;
  const count = passes + results.failures.length;
  console.log(`passed ${passes} out of ${count}`);
